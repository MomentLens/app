...
4. **Every edge case, by category.** Go through each of these and give at least one case, or say why the category cannot apply to this slice:
   ...
   For each case, say what the docs say to do, citing the id, or say **"the docs do not say"**. Never fill a gap with a guess here; naming the gap is the work. Read the spec §5 subsections for the slice's area; `node scripts/doc.mjs spec §5` lists all six.
...
One package per session, in the card's build order: `/slice S-12 build api`, `/clear`, then `/slice S-12 build mobile`. Load the card and confirm the schema PR has merged. Read that package's `AGENTS.md`, the schema files the card names, by path, and the files you will change. Fetch a doc section only if the card lists its id.
