...
8. **Do Not Publish activation is blocked without at least one accepted reference**, a reference or profile photo in which the worker found exactly one face, and the last one cannot be deleted while Do Not Publish is active. No reference embedding means the flag protects nobody while the UI reads "Active." (D-56, D-87, D-91)
