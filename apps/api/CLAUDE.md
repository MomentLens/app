...
- **`face_reference`**: the owner, and only their own rows. Once their subject has Do Not Publish active, no reference photo is presigned for anyone, the owner included, and the list returns each row's status with no URL (D-109). Embeddings never leave the database and the worker.
