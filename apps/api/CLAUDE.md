...
Auth middleware verifies the Supabase JWT with supabase-js `getClaims`, which checks it against the project's cached signing keys, and attaches the user to `req.user` (D-109). Do not hand-roll JWT verification. Nothing downstream re-checks identity.
...
- **`face_reference`**: the owner, and only their photos, with or without Do Not Publish (D-109). Embeddings never leave the database and the worker.
- **`venue.qr_secret`**: the event's Admin only (D-17).
- **`profile.avatar_key`**: never presigned for a user whose subject has Do Not Publish active, for anyone but that user, the Admin included (D-109). The app shows a name-initial placeholder instead (D-35). One function presigns avatars, and every endpoint that returns a person calls it.
...
2. SHA-256 match against existing media for this event. A row with this hash and `uploaded_at` set, deleted or not, is a duplicate: reject it silently, no file transfer, no prompt. The caller's own row with no `uploaded_at` is a crashed upload: re-sign its existing keys and stop. Another user's unfinished row is not a duplicate (D-96).
3. For a new row only: verification, meaning a `venue_verification` row for this user and sub-event OR `membership.admin_verified_at IS NOT NULL` OR `role = 'photographer'`. Then the cap, inside `start_upload` (below).

Then build the upload keys for the photo and its thumbnail in the one key function and call the `start_upload` SQL function with `rpc`. It locks the event row, refuses the 2,001st media row that is not soft-deleted, finished or not, and inserts the row with its keys (D-95). Presign a PUT URL for each key with `@aws-sdk/s3-request-presigner`, living 15 minutes (D-70, D-105). Pre-flight takes the photo's capture time, and uses the pre-flight time when the photo has none (D-98).

On the client's completion call, send R2 a HEAD for both objects and answer 409 `upload_missing` if either is absent. Then call `complete_upload` with `rpc`. It checks the caller uploaded the row, sets `uploaded_at` where it is null, stores `size_bytes` from the HEAD, and sends exactly one message to the `jobs` queue, all in one transaction: `thumbnail_dims` until S-21, `face_process` from then on. Never both (D-72). A repeated call changes nothing and answers `completed`. When another finished row already has the hash, it deletes this row and answers `duplicate`; delete the row's two objects in R2. Two supabase-js calls in a row look the same and lose the job on a crash between them. Status codes and error bodies are Handbook §5.3, and `docs/ARCHITECTURE.md` §4 has what each answer does to the phone's queue.
