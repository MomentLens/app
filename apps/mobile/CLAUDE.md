...
**Builds are 64-bit only.** `expo-build-properties` sets `buildArchs` to `arm64-v8a` and `x86_64`, so every build, local or EAS, skips the two 32-bit variants nobody on this team owns. That halves the native compile and the APK, and it is why a build needs no architecture flag on the command line. `arm64-v8a` is every phone here and the Apple Silicon emulator; `x86_64` is the emulator on an Intel laptop, which Maestro needs in Phase 7. A 32-bit-only Android device cannot install the result. If one ever has to, add the ABI here rather than passing a flag, or that device works for whoever remembered the flag and for nobody else.
