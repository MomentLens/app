...
<!-- doc:toc begin -->
<!-- generated by scripts/docindex.mjs, 26 chunks. Do not edit by hand.
     Retrieval: node scripts/doc.mjs <id>   e.g. doc D-01

momentlens-architecture  5439 tok  MomentLens architecture
arch §1            563 tok  Data access — Decided in D-73.
who-may-see-what   348 tok  Who may see what — The API enforces every rule here. The two marked rows are also RLS policies.
arch §2           1362 tok  Tables — Planned, not migrated, except health_check.
profile             43 tok  profile — One row per auth user, keyed by user_id.
push_token          12 tok  push_token — user_id, expo_push_token
subject             99 tok  subject — The identity that reference photos and Do Not Publish attach to, split from the account in the first migration (
face_reference     119 tok  face_reference — subject_id, source (profile, reference, auto_added), is_curated (D-54), embedding vector(512)
event               76 tok  event — name, type, description, cover_key
venue               37 tok  venue — event_id, name, lat, lng, qr_secret
sub_event           80 tok  sub_event — event_id, name, description, starts_at, ends_at, venue_id
membership         103 tok  membership — event_id, user_id, unique together
invite              46 tok  invite — event_id, role (guest, photographer), token, shortcode (6 characters), revoked_at
venue_verification    67 tok  venue_verification — The spec's VenueVerification, renamed to the naming convention.
media              134 tok  media — event_id, sub_event_id, uploader_user_id, uploader_role_at_upload (display only, D-13), captured_at
face                88 tok  face — One row per detected face, written only by the worker.
dnp_subject         67 tok  dnp_subject — A Do Not Publish subject found in a photo, with that subject's personalized files.
blur_request       122 tok  blur_request — face_id, requester_subject_id, similarity against curated references only (D-54), auto_reference_id
photo_flag          24 tok  photo_flag — media_id, flagged_by_user_id, state (open, kept, removed) (§2.5)
consent             36 tok  consent — user_id, policy_version, accepted_at.
health_check       147 tok  health_check — Infrastructure, not a feature table. sql.
arch §3            385 tok  R2 object keys — Two buckets, momentlens-dev and momentlens-stable, one per Supabase project, so no row ever points at ano
arch §4            280 tok  Upload pipeline — Spec §4.8, Handbook §7.
arch §5            553 tok  Worker jobs — Five pgmq jobs: thumbnail_dims, face_process, reference_process, reprocess and manual_blur, with their trigg
arch §6            210 tok  Similarity thresholds — Not measured. Never use a number from any spec draft (Handbook §11).
arch §7           1195 tok  Deployment — Development and demo side by side: Netcup server from 2026-10-15, Oracle trial until then, the two systemd un
-->
<!-- doc:toc end -->
