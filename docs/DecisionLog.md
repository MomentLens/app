...
**Reopen if.** The corpus outgrows a linear scan, which at 300 chunks and roughly 40ms it is nowhere near, or the team stops running `doc` and goes back to grep, which would mean the command is not earning its place.
