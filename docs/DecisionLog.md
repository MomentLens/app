...
**Measured.** Over all 41 slices, with `cl100k_base` and the tool-call framing counted as well as the text: 64,000 tokens and 41 calls through `doc slice`, against 106,000 and 136 calls retrieving the same content by hand from someone who already knows every section number, and 158,000 and 286 calls following the heading-listing recipe this replaced. A third of the hand-retrieval gap is the tool calls themselves. The printed `~N tok` figures are an estimate with no dependency, fitted to this corpus against `cl100k_base`: 85% of chunks land within 10%, the corpus total within 0.1%.
