...
<!-- doc:toc begin -->
<!-- generated by scripts/docindex.mjs, 97 chunks. Do not edit by hand.
     Retrieval: node scripts/doc.mjs <id>   e.g. doc D-01

momentlens-decision-log  3640 tok  MomentLens decision log
how-to-use-this-file   321 tok  How to use this file — Every entry records a decision, the reasoning behind it, and what was rejected and why.
a-scope-and-product  1390 tok  A. Scope and product
D-01               309 tok  Personalized face blur is core scope, not a stretch goal — The Do Not Publish subject sees their own face unblurred while 
D-02               296 tok  Sub-event visibility groups are cut ⚠ — No per-sub-event restricted albums in v1.
D-03               201 tok  Burst grouping and best-shot selection stay a stretch goal — Not core scope.
D-04               209 tok  Full-album ZIP export is out; multi-select download is the primary export — Users select photos and save them to the devic
D-05               160 tok  Sharing links and a web viewer are stretch, not core — In v1, anyone who wants to see the album joins the event as a Guest
D-06               210 tok  Pinch-zoom and pan in the photo viewer are deferred (SUPERSEDED by D-59) — The full-screen viewer supports swipe between p
b-roles-and-permissions  1451 tok  B. Roles and permissions
D-07               279 tok  Photographer media goes straight into the shared album — Photographer uploads are visible to every event member by default
D-08               229 tok  The Photographer is write-only and cannot read the album ⚠ — They see only their own uploads in My Media.
D-09               101 tok  The Photographer keeps read-only access to the Schedule tab — They can see sub-event names, times, and venues.
D-10               198 tok  Photographers are exempt from the location verification gate — The pre-flight verification check passes automatically for 
D-11               284 tok  Photographers upload at full quality; the worker generates the display variant (SUPERSEDED by D-58) — No client-side 2048p
D-12               152 tok  Album-closed applies to Photographers too — No role bypasses album state, including Admin and Photographer.
D-13               201 tok  There is no role-based media visibility rule at all — All uploaded photos are visible to all event members regardless of t
c-location-verification  1013 tok  C. Location verification
D-14               277 tok  Verification is a blocking gate, not a visible flag ⚠ — Unverified photos do not upload. They wait in the local queue.
D-15               225 tok  Verification granularity is per sub-event, with a blunt admin override — A VenueVerification row is scoped to one user and
D-16               207 tok  GPS verification runs on-device; the server re-validates — The client compares GPS against cached sub-event coordinates lo
D-17               152 tok  The Venue QR works offline without pre-caching any secret — A scan is written to local SQLite and travels with the next pr
D-18               145 tok  Retroactive per-sub-event verification requests are deferred — No flow where a user asks the Admin to verify them for a su
d-capture          848 tok  D. Capture
D-19               233 tok  A sub-event stays In Progress until the next one starts — Status ignores the sub-event's own scheduled end time.
D-20               163 tok  The capture FAB stays hidden when no sub-event is In Progress — Capture is available only during a live sub-event.
D-21               143 tok  Capture keeps the camera's native aspect ratio — No forced 4:3 or any other crop.
D-22               306 tok  FlashList grid, not masonry, but store dimensions anyway — Square-cropped uniform grid. The worker writes width and height
e-face-processing-and-blur  1723 tok  E. Face processing and blur
D-23               330 tok  Manual self-blur is gated by an embedding check, not by Admin approval — When a user taps their own face to blur it, compa
D-24               147 tok  The blur applies immediately and the Admin reverts, rather than approving first — A queued blur request is already applied
D-25               174 tok  A confirmed face crop becomes a new reference embedding — When a user taps their own face, that crop is added to their ref
D-26               202 tok  The self-visible marker is mandatory, not decoration — When a Do Not Publish user views their own unblurred crop, a lock i
D-27               214 tok  Retained originals are load-bearing, not speculative — The pre-blur uploaded file stays in R2 and is the source every blur
D-28               284 tok  Downloads are personalized, so DNP photos leave the public CDN path (SUPERSEDED by D-57) — A Do Not Publish subject downlo
D-29               191 tok  One detection pass, with an explicit filter — Detect faces once, on the original. Build the Recognized Faces list from tha
D-30                78 tok  The blur pipeline runs on every upload regardless of role — Detection and blur run on Photographer uploads too, and blur b
D-31                96 tok  Do Not Publish is permanently irreversible; a single manual blur is not — Two different objects. Enabling DNP cannot be un
f-data-and-storage   398 tok  F. Data and storage
D-32               282 tok  SHA-256 content hash replaces pHash — Hash with SHA-256. Exact match is rejected at pre-flight via one indexed lookup.
D-33               110 tok  Guest cap 150, upload cap 2,000 — Down from 500 and 10,000.
g-privacy-posture   576 tok  G. Privacy posture
D-34               156 tok  "Private mode" is renamed Local Only and writes to the app sandbox — documentDirectory, not the camera roll, with the do-n
D-35               128 tok  Do Not Publish hides the profile photo, not the name — The image is replaced by a name-initial placeholder everywhere, inc
D-36               129 tok  GPS is transmitted for verification and not persisted — Stripped from the image file. A separate reading rides with the pr
D-37               158 tok  Known Limitations are written down rather than hidden — The spec carries a section stating plainly that DNP only protects 
h-infrastructure  1006 tok  H. Infrastructure
D-38               520 tok  One Oracle Always Free ARM instance, no provider rotation — Flex in Singapore, permanently. One team member's Azure studen
D-39               205 tok  No Docker; systemd, nginx, and certbot instead — Express and the worker run as systemd units behind nginx, with TLS from c
D-40               130 tok  The InsightFace model loads once at worker startup — Resident in memory, never lazy-loaded per job.
D-41               146 tok  React Native over Flutter — React Native with Expo.
i-process         1609 tok  I. Process
D-42               111 tok  Retention windows live in one table — Photo soft delete 30 days, event soft delete 14 days, album visibility 30 days after
D-43                75 tok  Two notification channels, not four — Approval Alerts and Album Lifecycle only.
D-44               171 tok  The demo script is the scope boundary — An eight-beat script lives in the spec, and anything not in it is not core scope.
D-45               379 tok  Every AI-generated line must be explainable by a human on the team (SUPERSEDED by D-68) — Nothing merges if none of the th
D-46               285 tok  The Do Not Publish recognition filter is viewer-scoped, not global — A face matched to a Do Not Publish user is hidden fro
D-47               314 tok  The Admin's inability to verify a blur requester's identity is accepted, not solved ⚠ — When a low-confidence manual blur 
D-48               271 tok  Full-quality download of photographer originals is deferred (VOID, see D-58) — Every download in v1 serves the 2048px vers
j-applied-from-the-resolution-log-spec-v11  4662 tok  J. Applied from the resolution log (spec v11) — Two constraints settled a third of these before 
D-49               208 tok  No real event is ever covered; the dataset is ~100 seeded photos — Development and the demo run on roughly 100 photos at 1
D-50               503 tok  The demo backend runs on the M1; the Oracle instance is provisioned anyway (SUPERSEDED by D-78) — Express and the FastAPI 
D-51                58 tok  ngrok is not the tunnel (SUPERSEDED by D-78) — Cloudflare Tunnel with a named hostname.
D-52               292 tok  Tap-to-blur is available only to users with Do Not Publish active — The manual correction affordance renders only for user
D-53               160 tok  Hash the bytes being uploaded, not a re-encoded thumbnail — SHA-256 over the exact byte stream the client is about to PUT,
D-54               207 tok  Curated and auto-added references are tracked separately — One boolean column. Matching and Find My Photos use curated plu
D-55               180 tok  A media row is not album-visible until processing completes — The album query filters on processed_at IS NOT NULL.
D-56               172 tok  Do Not Publish cannot be activated without a reference image — Activation is blocked unless the user has at least one refe
D-57               373 tok  Personalized variants replace the crop-and-overlay design ⚠ — For a photo with N Do Not Publish subjects the worker writes
D-58               234 tok  No client-side resize, with a 4096px guard — Phone JPEGs upload as they are. If the longest edge exceeds 4096px, resize to
D-59               148 tok  Pinch-zoom and pan are restored — The full-screen viewer supports pinch-zoom and pan.
D-60               265 tok  Blur variant object keys carry a version ⚠ — jpg, with variant_version as an integer column on the media row, bumped on ev
D-61               238 tok  Judges use team-owned Android devices; there is no deferred deep link ⚠ — Demo beat 2 runs on team-owned Android devices h
D-62               121 tok  The offline fallback is a recording, not a seeded dataset — A recorded walkthrough of the full script on local storage, on
D-63               342 tok  Proxy Blur is designed and deferred; the schema split happens now — Not built. Written into spec §6.2 in full.
D-64               158 tok  Manual blur rate limiting is designed and deferred — Not built in v1. Written into spec §6.2: cap at 10 requests per user 
D-65               141 tok  Blur geometry and blur strength are specified, not left to the implementer — Expand the detection box by 30 to 40 percent,
D-66               128 tok  Retroactive reprocessing never re-runs detection — The reprocess job compares stored embeddings against the newly-active r
D-67                92 tok  The Supabase keep-alive runs from GitHub Actions — A scheduled workflow, not a cron on the compute box.
D-68               589 tok  Code comprehension is not a merge gate, and simplicity is not an instruction to the agent — Supersedes D-45.
k-found-while-scaffolding-the-repo-2026-09-13  1408 tok  K. Found while scaffolding the repo (2026-09-13) — Four gaps found by reading the spec, handb
D-69               408 tok  The client thumbnail is served only for photos with no Do Not Publish face ⚠ — 1 Stage 1) and PUTs it straight to R2 with 
D-70               279 tok  R2 keys have one builder per key family — Replaces the "one place" rule in Handbook §3 and root invariant 12.
D-71               354 tok  Express queries Supabase as the caller (SUPERSEDED by D-73) — The auth middleware builds a Supabase client from the caller
D-72               307 tok  The thumbnail_dims warm-up job ships in Phase 3 — Slice S-18a builds the worker skeleton (pgmq consumer loop, /health) and
l-decided-while-completing-architecturemd-2026-09-13  1395 tok  L. Decided while completing ARCHITECTURE.md (2026-09-13) — The rule the team set for t
D-73               382 tok  Express uses the secret key, and RLS denies direct access ⚠ — Supersedes D-71. The API queries Supabase with the secret ke
D-74               233 tok  The worker does all face matching and stores the results — Embeddings are stored in pgvector vector(512) columns.
D-75               228 tok  Ukasha owns ARCHITECTURE.md, and agents write it — Settles the ownership open item and replaces "you maintain it by hand" 
D-76               322 tok  Development runs on the Oracle instance; the M1 demo stack goes up a month before the demo — Amends D-50 and Handbook §13 
D-77               175 tok  Pinch-zoom is core, as a named exception to D-44 — Amends D-59 and D-44. The single photo viewer ships with pinch-zoom and
m-decided-after-benchmarking-the-worker-2026-09-16  1182 tok  M. Decided after benchmarking the worker (2026-09-16)
D-78               759 tok  One Netcup server runs development and the demo from 2026-10-15 ⚠ — Supersedes D-50 and D-51 and amends D-38 and D-76.
D-79               409 tok  The standby is a rotation across three student credits, and it runs only twice ⚠ — Amends D-38. The demo-week standby is n
n-decided-while-making-the-docs-addressable-2026-09-19  1656 tok  N. Decided while making the docs addressable (2026-09-19)
D-80               894 tok  The docs are retrieved by id through one command, not by grepping headings — Every heading in docs/ is an addressable chun
open-items-that-are-not-decisions-yet   747 tok  Open items that are not decisions yet — These are not settled and should not be treated as though the
-->
<!-- doc:toc end -->
