...
<!-- doc:toc begin -->
<!-- generated by scripts/docindex.mjs, 43 chunks. Do not edit by hand.
     Retrieval: node scripts/doc.mjs <id>   e.g. doc D-01

momentlens-engineering-handbook--v4 21801 tok  MomentLens engineering handbook — v4
hb §0              224 tok  How to use this document — Read §1 (stack), §2 (architecture), §8 or §9 (environment setup), and §18 (AI tools) once.
hb §1              817 tok  Tech stack at a glance
hb §2              657 tok  Software architecture overview — Three runtimes, one shared source of truth.
hb §3             1372 tok  Repository structure — One monorepo, three packages. For three people, a single repo beats three repos: no version-mismatc
hb §4             1047 tok  Frontend architecture — Navigation. Expo Router, file-based. tsx is the route /event/:id/album.
hb §5             1192 tok  Backend architecture (Express API) — Layering: route → controller → service. Routes define the HTTP surface, controllers p
hb §6             1048 tok  AI worker architecture (FastAPI) — This is a queue consumer, not a web server written in FastAPI.
hb §7              815 tok  The upload pipeline, architecturally — Worth its own section because getting this wrong is the easiest way to make a small
hb §8              498 tok  Development environment — macOS (Ukasha, M1) — Homebrew, if not already installed.
hb §9              354 tok  Development environment — Windows (x86, no GPU) — Same Node/pnpm/Android Studio/Python/EAS/VS Code steps as §8, with these
hb §10             408 tok  The fast iteration loop — This trips up almost every team new to React Native, so be explicit about the mental model: most
hb §11            1016 tok  Coding, building, and testing strategy — Linting and formatting: ESLint and Prettier for TypeScript, Ruff for Python.
hb §12             354 tok  Git & team collaboration — Trunk-based, not GitFlow. For three people, develop/release/hotfix branches are pure overhead.
hb §13            2901 tok  Deployment
hb §13.1           112 tok  Mobile app — EAS Build profiles in eas.json:
hb §13.2           307 tok  Backend: one server for development and the demo — Development and the demo run on one Netcup RS 1000 G12 root server from
hb §13.3          2476 tok  The server: one Netcup root server, no containers
hb §13.3.1         180 tok  Why no Docker — Why no Docker. The common argument against Docker on small hardware is that it is slow, and that argument 
hb §13.3.2         540 tok  Ordering and provisioning — Order it with room to test. Pick Ubuntu 24.04 LTS as the operating system.
hb §13.3.3         469 tok  The two systemd units — Two systemd units. sh writes these to /etc/systemd/system/.
hb §13.3.4         234 tok  nginx and TLS — nginx as reverse proxy, in /etc/nginx/sites-available/momentlens:
hb §13.3.5        1039 tok  Deploying an update — sh is this sequence, and running that script is how you deploy.
hb §14            2782 tok  Things to build first — The single most important idea here: build one thin slice through the entire system before buildin
hb §14.0           445 tok  Phase 0 — plumbing (days, not weeks) — Environment on all three machines (§8/§9).
hb §14.1            75 tok  Phase 1 — walking skeleton — Auth (sign up, log in via Supabase Auth), create an event with bare-minimum fields, join via 
hb §14.2            76 tok  Phase 2 — event structure — The informational schedule with the §4.3 status computation, both role-specific invite links, 
hb §14.3           205 tok  Phase 3 — capture & upload — The Viewfinder (native aspect, 1x fixed, no gallery picker), My Media with its SQLite queue, 
hb §14.4            90 tok  Phase 4 — location verification gating — Add the on-device GPS check, the server-side re-validation, the queue gate, the p
hb §14.5           868 tok  Phase 5 — the AI worker and face blur — The warm-up job changed, because D-58 deleted the old one.
hb §14.6           149 tok  Phase 6 — the rest — Notifications (Approval Alerts and Album Lifecycle only).
hb §14.7           740 tok  Phase 7 — buffer. Reserve it and defend it — Testing pass, performance pass (§16), UI polish, and demo rehearsal on real d
hb §15             893 tok  UI design direction — The photos are the product. The chrome around them should recede, not compete.
hb §16            1149 tok  Performance: keeping the UI thread unblocked — The current model, accurately. React Native's New Architecture (JSI, Fabric
hb §16.5           424 tok  Implementing the navigation architecture (spec §2.5) — The two-tier navigation model is the most architecturally significa
hb §17             325 tok  Quick reference: full library list
hb §18            2073 tok  Using AI coding tools without wrecking the project — You are three people with limited experience building something that 
hb §18.1           594 tok  The two rules — Rule 1: the dangerous surfaces get read before they merge.
hb §18.2           508 tok  Managing context, practically — Context is the scarcest resource in an agent session and most people waste it on the wrong
hb §18.3           244 tok  What to delegate, and what not to — Good delegation. Boilerplate CRUD screens and endpoints.
hb §18.4           175 tok  The version-drift problem, specifically — Your stack moves faster than model training data.
hb §18.5           163 tok  Commit hygiene for agent-generated code — Commit in small pieces even when the agent produced 400 lines at once.
hb §18.6           136 tok  One thing that is easy to miss — One use of these tools is badly underrated here: adversarial review of code that already 
-->
<!-- doc:toc end -->
