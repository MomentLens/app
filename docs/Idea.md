...
<!-- doc:toc begin -->
<!-- generated by scripts/docindex.mjs, 76 chunks. Do not edit by hand.
     Retrieval: node scripts/doc.mjs <id>   e.g. doc D-01

momentlens--complete-application-flow--feature-reference 26311 tok  MomentLens — complete application flow & feature reference
table-of-contents   186 tok  Table of contents — What changed in v11
spec §0           1060 tok  What changed in v11 — v11 applies the resolution log. Two constraints drove most of it, and they belong before the change 
spec §1            194 tok  User roles overview
spec §2           5469 tok  Complete application flow
spec §2.1          887 tok  Admin flow
spec §2.1.1         66 tok  Phase A — account setup
spec §2.1.2        193 tok  Phase B — creating an event
spec §2.1.3        361 tok  Phase C — invitation & venue QR
spec §2.1.4        167 tok  Phase D — during the event
spec §2.1.5         95 tok  Phase E — post-event
spec §2.2          315 tok  Photographer flow
spec §2.3          700 tok  Guest flow
spec §2.3.1        223 tok  Phase A — joining
spec §2.3.2         72 tok  Phase B — pre-event
spec §2.3.3        342 tok  Phase C — event day
spec §2.3.4         59 tok  Phase D — post-event
spec §2.4          319 tok  New user flow
spec §2.5         3240 tok  Navigation & screen architecture
spec §2.5.1        406 tok  Two-tier tab bar — The bottom tab bar is not one static set of tabs.
spec §2.5.2        466 tok  Home (event) — merged home + album — Home and Album are one screen, not two, and not per-sub-event folders.
spec §2.5.3        320 tok  My media — Sectioned by sub-event, the same visual pattern as the Home grid (sticky header with numeral, sub-event name, d
spec §2.5.4        318 tok  Camera FAB & viewfinder — FAB (camera icon, bottom-right) renders on My Media for Guest and Admin.
spec §2.5.5         97 tok  Schedule — One list component reused for every role, permission-gated rather than forked: ordered sub-events with an auto-
spec §2.5.6        440 tok  Single photo view — Full-screen swipeable pager. Tap toggles the metadata overlay (capture time, sub-event, uploader).
spec §2.5.7        439 tok  Manage (admin only) — Grouped hub screen, iOS-Settings-style list of rows each linking to its own sub-screen, not one long
spec §2.5.8        235 tok  Screens formalized in this pass
spec §2.5.9        357 tok  Account settings (global, §4.19) — Reachable from the Profile tab and, redundantly, from an avatar icon in the Event shell
spec §2.5.10       151 tok  Notification deep-links — Push-only. There is no in-app history screen; §6.2 defers a Notification Center.
spec §3            771 tok  Event lifecycle (end to end) — One event walked from creation through invitation, the event day, upload and processing, to
spec §4          11409 tok  Complete feature list
spec §4.1          351 tok  Authentication & onboarding — Email address and password registration via Supabase Auth.
spec §4.2          435 tok  User profile — Full name and profile photo. No bio field (§6.1).
spec §4.3          383 tok  Event management — Create event: name, type, cover photo, description.
spec §4.4          285 tok  Invitation & access control — Guest Link and Photographer Link, each a distinct token carrying its own role assignment, so
spec §4.5          590 tok  Location verification & attendance — This system is a gate on uploading, applied to the person, not the photo.
spec §4.6           59 tok  Event schedule — A simple, informational, ordered list of sub-events: name, date/time, venue, "Get Directions."
spec §4.7          332 tok  Media capture (viewfinder) — A custom in-app camera Viewfinder, not the OS native camera.
spec §4.8          745 tok  Upload pipeline
spec §4.8.1        340 tok  Stage 1 — client-side processing — One pipeline for every role. v10 branched on role to skip the resize for Photographers.
spec §4.8.2        212 tok  Stage 2 — pre-flight — A single small JSON round-trip: content hash, album ID, sub-event ID, and the GPS reading captured 
spec §4.8.3        187 tok  Stage 3 — upload — Direct to R2 via the presigned URLs, one for the photo and one for its thumbnail.
spec §4.9          768 tok  Album — The album is a single shared view for every Guest and the Admin.
shared-album       404 tok  Shared album — Grid view, grouped by sub-event and date, not by uploader.
album-state        301 tok  Album state — Upload access is enabled or disabled purely by the Admin's manual open/close toggle.
spec §4.10         406 tok  Photographer workflow — Write-only contributor. Their uploads land in the shared album immediately and are visible to ever
spec §4.11        3609 tok  AI media intelligence
spec §4.11.1        59 tok  Deduplication — SHA-256 over the exact byte stream the client uploads (§4.8).
spec §4.11.2       588 tok  Face detection & embedding, the shared foundation — One detection pass per photo, on the uploaded file.
spec §4.11.3       460 tok  Face recognition: named & unknown clustering — Find My Photos: one tap, using the reference embeddings already on the user
spec §4.11.4      2494 tok  Do Not Publish: personalized face blurring — This is the app's centerpiece feature and it is core scope, not a stretch goa
spec §4.11.4.1     333 tok  Activation and the files generated per photo — Activation requires at least one reference image.
spec §4.11.4.2     463 tok  Serving, and the self-visible marker — Serving is one endpoint, and the client never chooses.
spec §4.11.4.3     149 tok  Blur geometry and strength — Blur geometry and blur strength are specified, not left to whoever writes the worker.
spec §4.11.4.4     966 tok  Correcting a missed match — Correcting a missed match.
spec §4.11.4.5     550 tok  Retroactive reprocessing — Retroactive reprocessing is a match job, never a detection job.
spec §4.12         327 tok  Local Only mode — Renamed from "Private mode," because the previous design saved to the device camera roll, which is the m
spec §4.13         587 tok  Media delivery — 300px WebP thumbnails via expo-image, center-cropped to square for grid uniformity across mixed-aspect so
spec §4.14         255 tok  Offline mode — Cached on first load: the schedule, sub-event and venue details including coordinates and radius, and the t
spec §4.15         204 tok  Post-event features — Multi-select download to the device gallery, plus individual photo download.
spec §4.16         137 tok  Notifications
spec §4.17         243 tok  Limits (hard-coded, no subscription system) — There is no PlanTier concept in v1. Subscriptions are entirely Future Work (
spec §4.18         342 tok  Privacy & consent management — Explicit consent screen on first launch, covering:
spec §4.19         271 tok  Settings & preferences — Account: update profile photo, manage reference photos (up to 5), request account deletion (conta
spec §4.20         874 tok  Deployment & hosting — One compute environment for development and the demo, and the viva will ask why.
spec §4.21         202 tok  Retention & deletion windows — Consolidated here because four scattered numbers in four sections is how a three-person tea
spec §5           1229 tok  Edge cases & exception handling — What the app does when things go wrong: failed uploads, lost connectivity, duplicate joi
spec §6           1004 tok  Deferred & out of scope
spec §6.1           40 tok  Not planned (permanently out of scope) — Reverting Do Not Publish. Face-recognition opt-out.
spec §6.2          957 tok  Future work (post-FYP roadmap) — Video support, including face blur for video, which is materially harder than the photo c
spec §7            535 tok  If time allows (stretch goals this FYP) — Build these only after the core scope above is working, tested, and demo-stable.
spec §8           1104 tok  Known limitations (state these, don't hide them) — Every one of these will be asked about in the viva.
spec §9           1233 tok  The demo script — Roughly 30 minutes, a university classroom, 3 team members and 3 to 4 judges, campus WiFi, GPS verified 
before-the-room-what-has-to-already-be-true   349 tok  Before the room: what has to already be true — Devices. The app is not on any store and there i
the-beats          587 tok  The beats — Create the event. Two sub-events, one currently live.
two-answers-to-have-ready-because-they-will-be-asked   201 tok  Two answers to have ready, because they will be asked — " One server ran development a
-->
<!-- doc:toc end -->
