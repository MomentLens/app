...
#### Screens formalized in this pass
...
#### Face detection & embedding, the shared foundation
...
- **Reference sets are split in two, and the split is load-bearing.** A user may upload up to 5 reference photos of themselves, and their profile photo counts as one if it exists (§4.2). Those are **curated** references. A face crop added automatically by the correction flow below is an **auto-added** reference.
...
#### Face recognition: named & unknown clustering
...
#### Do Not Publish: personalized face blurring
