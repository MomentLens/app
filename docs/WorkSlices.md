...
**S-20 also builds setting a profile photo**, for the signup step in spec §2.1.1 and for S-29's Settings row. Setting one can create the subject and the `profile` reference with its `reference_process` message, so it is one SQL function (arch:face_reference, D-95). S-01 left it out. While Do Not Publish is active, the reference list shows the placeholder in place of the `profile` reference's photo (arch §1).
